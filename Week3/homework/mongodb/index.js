...
async function findEpisodesExercises(client) {
  ...
  const result1 = await bobRossCollection.findOne({ episode: "S02E02" });

  console.log(`The title of episode 2 in season 2 is ${result1.title}`);

  // Find the season and episode number of the episode called "BLACK RIVER" [Should be: S02E06]
  const result2 = await bobRossCollection.findOne({ title: "BLACK RIVER" });
  ...
  const result3 = await bobRossCollection.find({ elements: "CLIFF" }).toArray();
  const cliffResult = result3.map((e) => e.title);

  console.log(`The episodes that Bob Ross painted a CLIFF are ${cliffResult}`);

  // Find all of the episode titles where Bob Ross painted a CLIFF and a LIGHTHOUSE [Should be: NIGHT LIGHT]

  const result4 = await bobRossCollection
    .find({ $and: [{ elements: "CLIFF", elements: "LIGHTHOUSE" }] })
    .toArray();
  const cliffLightResult = result4.map((e) => e.title);

  console.log(
    `The episodes that Bob Ross painted a CLIFF and a LIGHTHOUSE are ${cliffLightResult}`
  );
}
...
async function deleteEpisodeExercise(client) {
  ...
  const result1 = await bobRossCollection.deleteOne({ episode: "S31E14" });

  console.log(
    `Ran a command to delete episode and it deleted ${result1.deletedCount} episodes`
  );
}
